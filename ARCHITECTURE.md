...
┌─────────────────────────────────────────────────┐
│                   请求入口层                       │
│   Pydantic 校验 (@validate_request)              │
│   SSE 流式输出 (sse_stream)                      │
├─────────────────────────────────────────────────┤
│                  业务代码层                        │
│   routes / services / schemas / worker_engine    │
├─────────────────────────────────────────────────┤
│                  统一接口层                        │
│   db (SQLAlchemy) · redis_client (PubSub)        │
│   task_queue · executor                          │
├──────────────────────┬──────────────────────────┤
...
`WorkerConfig` 提供任务重试策略配置。`RedisKeyManager` 集中管理 Redis key 的命名规范，包括任务状态 key（`task:{id}`）、取消标记 key（`cancel:task:{id}`）和 SSE 流 channel（`stream:task:{id}`）。

### schemas.py — Pydantic 数据模型

集中定义所有请求和响应的数据结构，基于 Pydantic v2：

- **`ItemCreate`** — 创建请求校验，`title` 必填（1-200 字符），`description` 可选
- **`ItemResponse`** — 响应模型，配置 `from_attributes = True` 支持直接从 SQLAlchemy ORM 对象构造
- **`ErrorResponse`** — 统一错误响应格式

响应序列化使用 `model.model_dump(mode="json")`，确保 `datetime` 等类型正确转换为 JSON 可序列化格式。

### utils/validation.py — 请求校验装饰器

`@validate_request(SchemaClass)` 装饰器将 Pydantic 校验集成到 Flask 路由中：

```
HTTP Request → @validate_request(Schema)
                       │
               request.get_json()
                       │
              ...
              校验通过      校验失败
                 │           │
          body 注入 kwargs   422 + 字段级错误详情
                 │
            路由函数执行
```

校验失败时返回结构化错误响应：`{"error": "validation failed", "details": [{"field": "title", "message": "Field required"}]}`。

### utils/sse.py — SSE 流式输出

`sse_stream(channel)` 生成器函数，订阅 PubSub channel 并持续 yield SSE 格式数据。核心机制：

1. 通过 `redis_client.pubsub()` 创建订阅（Desktop 用 MemoryPubSub，Server 用 Redis PubSub）
2. 以 1 秒间隔轮询 `get_message()`
3. 收到消息时格式化为 `data: {...}\n\n` 并 yield
4. 每 15 秒发送 `: heartbeat\n\n` 保持连接活跃
5. 30 秒无消息或收到终态（`completed` / `failed`）时自动关闭流
6. `finally` 块确保 unsubscribe 清理

`format_sse(data, event, id)` 辅助函数处理 SSE 协议格式，支持可选的 event type 和 event id。
...
1. 通过 ID 从数据库加载实体
2. 更新状态为 `processing`，通过 PubSub 发布事件
3. 执行业务逻辑
4. 更新状态为 `completed` 或 `failed`，通过 PubSub 发布事件

每次状态变更都会调用 `_publish_status()`，向 `RedisKeyManager.stream_channel(item_id)` 发布 JSON 事件。SSE 端点订阅同一 channel，实现实时推送。
...
以"创建 Item 并监听处理进度"为例，展示各模块如何协作：

```
1. POST /api/items  {"title": "test"}
   │
   ├─ @validate_request(ItemCreate)     ← utils/validation.py
   │    └─ Pydantic 校验通过 → body: ItemCreate
   │
   ├─ Item(...) → db.session.commit()   ← routes/example.py
   │
   └─ 返回 ItemResponse.model_dump()    ← schemas.py
       → 201 {"id":1, "title":"test", "status":"pending", ...}

2. POST /api/items/1/process
   │
   ├─ item.status = "queued"
   │
   └─ task_queue.enqueue("backend.worker_engine.process_task", 1)
       │
       ├─ Desktop: MemoryQueue → 线程内执行
       └─ Server:  RQ Queue → Redis → Worker 进程执行

3. GET /api/items/1/stream  (SSE 长连接)
   │
   ├─ subscribe("stream:task:1")        ← utils/sse.py
   │
   ├─ Worker 执行 process_task(1):
   │    ├─ status → "processing"
   │    │   └─ publish("stream:task:1", {"status":"processing",...})
   │    │       → SSE: data: {"status":"processing",...}
   │    │
   │    └─ status → "completed"
   │        └─ publish("stream:task:1", {"status":"completed",...})
   │            → SSE: data: {"status":"completed",...}
   │
   └─ 流自动关闭，unsubscribe 清理
```
